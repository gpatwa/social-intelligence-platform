...
"""Register the desired state for a Page-linked Instagram Graph API source."""
...
dbutils.widgets.text("source_id", "instagram-graph-api")
dbutils.widgets.text("secret_scope", "external-github-actions")
dbutils.widgets.text("secret_key", "instagram-access-token")
...
spark.createDataFrame([Row(
    tenant_id=tenant_id, source_id=source_id, platform="instagram",
    connector_type="instagram_graph_api_page_linked", collection_mode="polling", enabled=True,
...
)]).createOrReplaceTempView("incoming_instagram_source")
spark.sql(f"""
    MERGE INTO {namespace}.`control_source_registry` AS target
    USING incoming_instagram_source AS incoming
...
rules = [("account", "linked_business_account")]
...
) for rule_type, expression in rules]).createOrReplaceTempView("incoming_instagram_rules")
spark.sql(f"""
    MERGE INTO {namespace}.`control_collection_rules` AS target
    USING incoming_instagram_rules AS incoming
...
""")
print(f"Registered Instagram source {tenant_id}/{source_id} with {len(rules)} active collection rule(s)")
